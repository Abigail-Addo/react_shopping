...
const Signup = () => {
  ...
  const defaultImageFilename = "user.jpg";
  ...
  const signinSubmit = async () => {
    try {
      let filename = defaultImageFilename;

      if (profileImage) {
        const formData = new FormData();
        formData.append("file", profileImage);
        let server = await fetch(
          "https://shopping-backend-mhxl.onrender.com/api/v1/store-image",
          {
            method: "POST",
            headers: {
      ...
      const result = await fetch(
        "https://shopping-backend-mhxl.onrender.com/api/v1/user",
        {
          method: "POST",
          headers: {
